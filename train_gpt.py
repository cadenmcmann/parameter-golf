...
class Hyperparameters:
    ...
    # Cache sidecar (evaluation-time)
    cache_enabled = bool(int(os.environ.get("CACHE_ENABLED", "0")))
    cache_size = int(os.environ.get("CACHE_SIZE", "8192"))
    cache_temperature = float(os.environ.get("CACHE_TEMP", "0.1"))
    cache_lambda_model = float(os.environ.get("CACHE_LAMBDA_MODEL", "0.85"))
...
class GPT(nn.Module):
    ...
    def forward_logits_and_hidden(self, input_ids: Tensor) -> tuple[Tensor, Tensor]:
        """Return (logits, hidden_states). logits: (bsz, seq_len, vocab). hidden: (bsz, seq_len, model_dim)."""
        n = self.num_layers
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
...
class NGramModel:
    """Count-based bigram/trigram model with additive smoothing."""

    def __init__(self, vocab_size: int, device: torch.device, hash_size: int = 16384):
        self.vocab_size = vocab_size
        self.bigram_counts = torch.zeros(vocab_size, vocab_size, dtype=torch.float32, device=device)
        self.trigram_counts = torch.zeros(hash_size, vocab_size, dtype=torch.float32, device=device)
        self.hash_size = hash_size
        self.smoothing = 1e-6

    def update(self, prev_tokens: Tensor, next_tokens: Tensor, prev_prev_tokens: Tensor | None = None) -> None:
        if prev_tokens.numel() == 0:
            return
        n = prev_tokens.shape[0]
        ones = torch.ones(n, device=prev_tokens.device, dtype=torch.float32)
        self.bigram_counts.index_put_((prev_tokens, next_tokens), ones, accumulate=True)
        if prev_prev_tokens is not None:
            h = (prev_prev_tokens.long() * 1009 + prev_tokens.long()) % self.hash_size
            self.trigram_counts.index_put_((h, next_tokens), ones, accumulate=True)

    def query(self, prev_tokens: Tensor, prev_prev_tokens: Tensor | None = None) -> Tensor:
        bi_counts = self.bigram_counts[prev_tokens] + self.smoothing
        p_bigram = bi_counts / bi_counts.sum(dim=-1, keepdim=True)
        if prev_prev_tokens is None:
            return p_bigram
        h = (prev_prev_tokens.long() * 1009 + prev_tokens.long()) % self.hash_size
        tri_counts = self.trigram_counts[h]
        has_trigram = tri_counts.sum(dim=-1) > 0
        tri_counts = tri_counts + self.smoothing
        p_trigram = tri_counts / tri_counts.sum(dim=-1, keepdim=True)
        alpha = 0.6
        return torch.where(
            has_trigram.unsqueeze(1),
            alpha * p_trigram + (1.0 - alpha) * p_bigram,
            p_bigram,
        )

# --- Sliding window evaluation ---

def eval_val_sliding_cache(
    args: Hyperparameters,
